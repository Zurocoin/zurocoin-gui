...
const char COMMUNITY_FORUM_URL[] = "https://zurocoin.ga";
const char REPORT_ISSUE_URL[] = "https://zurocoin.ga";
    ...
    , walletModel_(walletModel)
{
    ...
    setWindowIcon(QIcon(":images/zurocoin_lin"));
    clearTitle();
...
//    qDebug("raw tx = %s", qPrintable(tx.binary_transaction));
    ...
    for (const RpcApi::Transfer& tf: tx.transaction.transfers)
    {
        ...
        msg.append(tr("%1 ZRC to %2\n").arg(amountStr).arg(addressStr));
    }
    msg.append(tr("Fee: %1 ZRC\n").arg(formatAmount(tx.transaction.fee)));
    msg.append(tr("Total send: %1 ZRC").arg(formatAmount(-ourAmount)));
