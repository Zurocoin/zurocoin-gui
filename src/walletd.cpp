...
QString BuiltinWalletd::errorMessage(ReturnCode err)
{
    ...
    switch(err)
    {
    case ReturnCode::ZUROCOIND_DATABASE_ERROR:
        msg = tr("Database write error. Disk is full or database is corrupted.");
        break;
    case ReturnCode::ZUROCOIND_ALREADY_RUNNING:
        msg = tr("Cannot run zurocoind. Another instance of zurocoind is running.");
        break;
    case ReturnCode::WALLETD_BIND_PORT_IN_USE:
        msg = tr("Cannot run walletd. Walletd bind port in use.");
        break;
    case ReturnCode::ZUROCOIND_BIND_PORT_IN_USE:
        msg = tr("Cannot run zurocoind. Zurocoind bind port in use.");
        break;
    ...
    case ReturnCode::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Zurocoin wallet could not overwrite an existed file for safety reason. If you want to overwrite the file please remove it manually and try again.");
        break;
