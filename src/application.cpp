// Copyright (c) 2015-2018, The Bytecoin developers, The Zurocoin developers.
...
{
    setApplicationName("zurocoin"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Zurocoin Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("zurocoin-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Zurocoin wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Zurocoin wallet GUI already running"));
        return false;
    }
...
void WalletApplication::initSystemTrayIcon()
{
    ...
    m_systemTrayIcon->setIcon(QIcon(":images/zurocoin_lin"));
...
{
    splashMsg(tr("Looks like this is your first run of the new Zurocoin Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote walletd daemon."));
}
